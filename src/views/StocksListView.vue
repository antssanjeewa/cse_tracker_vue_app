...
const sourceStocks = computed(() => {
  if (viewMode.value === 'portfolio') {
    ...
    return marketStore.allStocks.filter(s => {
      const stockTicker = (s.code || '').toUpperCase()
...
<template>
  <DashboardLayout>
    <div class="flex flex-col h-[calc(100vh-64px)] overflow-hidden bg-[#0c1221]">
      <MarketToolbar v-model:searchQuery="searchQuery" v-model:viewMode="viewMode" :isLoading="marketStore.isLoading" @refresh="marketStore.fetchFullMarket" />

      <MarketFilterBar />

      <MarketTabs v-model:activeTab="activeColumnTab" :tabs="MARKET_TABS" />

      <MarketTable :columns="activeColumns" :stocks="paginatedStocks" :sortKey="sortKey" :sortDir="sortDir" :isLoading="marketStore.isLoading" :error="marketStore.error" :activeTab="activeColumnTab" @sort="sortBy" @refresh="marketStore.fetchFullMarket" @rowClick="goToStock" />

      <MarketPagination v-model:itemsPerPage="itemsPerPage" :totalItems="filteredStocks.length" :currentPage="currentPage" :totalPages="totalPages" :totalStocksCount="sourceStocks.length" @prevPage="prevPage" @nextPage="nextPage" @setPage="setPage" />
    </div>
  </DashboardLayout>
</template>
